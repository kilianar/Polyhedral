...
lemma salFinrank_eq_salFinrank_add_finrank_quot_linSpan {F G : PointedCone R M}
    (hF : F.IsFaceOf G) [Module.Finite R G.linSpan] :
    G.salFinrank = F.salFinrank + (G.quot F.linSpan).finrank := by
  letI : Module.Finite R F.linSpan :=
    PointedCone.moduleFinite_linSpan_of_le (R := R) (M := M) (F := F) (G := G) hF.le
  have hFG := PointedCone.finrank_eq_finrank_add_finrank_quot_linSpan
      (R := R) (M := M) (F := F) (G := G) hF.le
  rw [PointedCone.finrank_eq_finrank_lineal_add_salFinrank (R := R) (M := M) G,
    PointedCone.finrank_eq_finrank_lineal_add_salFinrank (R := R) (M := M) F,
...
lemma salFinrank_eq_salFinrank_add_salFinrank_quot_linSpan {F G : PointedCone R M}
  ...
  letI : Module.Finite R (G.quot F.linSpan).linSpan :=
    PointedCone.moduleFinite_linSpan_quot (R := R) (M := M) (F := F) (G := G)
  ...
  simpa [hq] using salFinrank_eq_salFinrank_add_finrank_quot_linSpan (R := R) (M := M) hF

/-- Dimension-addition for salient rank along a face inclusion (finite-dimensional case). -/
lemma salRank_eq_salRank_add_rank_quot_linSpan {F G : PointedCone R M}
    (hF : F.IsFaceOf G) [Module.Finite R G.lineal] :
...
lemma salRank_eq_salRank_add_salRank_quot_linSpan {F G : PointedCone R M}
  ...
  have hq : (G.quot F.linSpan).salRank = (G.quot F.linSpan).rank :=
    PointedCone.salRank_eq_rank_of_lineal_eq_bot
      (R := R) (M := M ⧸ F.linSpan) (C := G.quot F.linSpan) hqlineal
  simpa [hq] using salRank_eq_salRank_add_rank_quot_linSpan (R := R) (M := M) hF
